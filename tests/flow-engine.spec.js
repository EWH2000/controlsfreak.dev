...
// ── ⚠ TEMPORARY — delete with the ?gutter= flag ────────────────────
// PR-1 review scaffolding. Once the owner picks a variant, the flag,
// the unchosen variants and this test all come out together.
test('the ?gutter= review variants each behave as advertised', async ({ page }) => {
    const probe = async (variant) => {
        await page.setViewportSize(GUTTER_VIEWPORT);
        await page.goto(`/privacy.html?gutter=${variant}`);
        await expect(page.locator('.schematic-bg g.flow-particles circle').first()).toBeAttached();
        return page.evaluate(async () => {
            ...
            const read = () => cohort.map((c) => [c.cx.baseVal.value, c.cy.baseVal.value]);
            const before = read();
            await new Promise((r) => setTimeout(r, 400));
            const after = read();
            return {
                cohort: cohort.length,
                movers: before.filter((p, i) => Math.hypot(after[i][0] - p[0], after[i][1] - p[1]) > 0.5).length,
                // Placed but not moving is the v3 signature — proves the
                // engine ran and laid particles on their paths rather
                // than silently doing nothing.
                placed: document.querySelectorAll('.schematic-bg g.flow-particles circle[r="3"]').length,
                atOrigin: Array.from(document.querySelectorAll('.schematic-bg g.flow-particles circle[r="3"]'))
                    .filter((c) => c.cx.baseVal.value === 0 && c.cy.baseVal.value === 0).length,
            };
        });
    };

    for (const variant of ['v0', 'v1', 'v2']) {
        const r = await probe(variant);
        expect(r.cohort, `${variant}: gutter particles on screen`).toBeGreaterThan(4);
        expect(r.movers, `${variant}: the gutter animates`).toBe(r.cohort);
    }

    // v3 is the one variant that deliberately stops the gutter. Assert
    // BOTH halves — no motion, but the particles are still built and
    // placed on their paths. A broken engine also produces "no motion".
    const still = await probe('v3');
    expect(still.movers, 'v3: the gutter is static').toBe(0);
    expect(still.placed, 'v3: particles are still built').toBeGreaterThan(100);
    expect(still.atOrigin, 'v3: and placed on their paths, not left at 0,0').toBe(0);
});
