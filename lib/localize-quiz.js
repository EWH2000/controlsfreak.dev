...
function localizeQuiz(questions, locale, slug, translations) {
    ...
    questions.forEach((question) => {
        const label = `en/${slug}/${question.id}`;
        nonEmptyString(question.prompt, `${label}/prompt`);
        nonEmptyString(question.explain, `${label}/explain`);
        if (Object.prototype.hasOwnProperty.call(question, "snippet")) {
            nonEmptyString(question.snippet, `${label}/snippet`);
        }
        if (question.learnMore) {
            nonEmptyString(question.learnMore.href, `${label}/learnMore/href`);
            nonEmptyString(question.learnMore.label, `${label}/learnMore/label`);
        }
        (question.choices || []).forEach((choice) =>
            nonEmptyString(choice.text, `${label}/choices/${choice.id}`));
    });
    ...
    return questions.map((question) => {
        ...
        const localizedPrompt = localizeMarkupLinks(
            nonEmptyString(translated.prompt, `${selected}/${slug}/${question.id}/prompt`),
            selected
        );
        const localizedExplain = localizeMarkupLinks(
            nonEmptyString(translated.explain, `${selected}/${slug}/${question.id}/explain`),
            selected
        );
        const localized = {
            ...question,
        ...
        if (question.snippet) localized.snippet = localizeMarkupLinks(
            nonEmptyString(translated.snippet, `${selected}/${slug}/${question.id}/snippet`),
            selected
        );
        if (question.snippet) {
        ...
        if (question.learnMore) {
            localized.learnMore = {
                ...
                label: nonEmptyString(
                    translated.learnMoreLabel,
                    `${selected}/${slug}/${question.id}/learnMoreLabel`
                ),
            };
        }
        if (question.choices) {
